...
# ---------- HTML (UI kept as requested, updated formats) ----------
HTML = r"""
<!doctype html>
<html lang="en">
<head>
  ...
  <form id="frm">
    <div class="grid">
      ...
      <div><label>Format</label>
        <select id="format">
          <!-- VIDEO -->
          <option value="mp4_144" data-need-ffmpeg="1">144p MP4</option>
          <option value="mp4_240" data-need-ffmpeg="1">240p MP4</option>
          <option value="mp4_360" data-need-ffmpeg="1">360p MP4</option>
          <option value="mp4_480" data-need-ffmpeg="1">480p MP4</option>
          <option value="mp4_720" data-need-ffmpeg="1">720p MP4</option>
          <option value="mp4_1080" data-need-ffmpeg="1">1080p MP4</option>

          <!-- AUDIO MP3 BITRATES -->
          <option value="mp3_128" data-need-ffmpeg="1">MP3 128kbps</option>
          <option value="mp3_192" data-need-ffmpeg="1">MP3 192kbps</option>
          <option value="mp3_256" data-need-ffmpeg="1">MP3 256kbps</option>
          <option value="mp3_320" data-need-ffmpeg="1">MP3 320kbps</option>
        </select>
      </div>
...
def format_map_for_env():
    """
    Resolution-focused format selectors and audio bitrate keys.
    """
    if HAS_FFMPEG:
        return {
            # video-by-resolution -> combine video+audio and later merge with ffmpeg
            ...
            # audio keys use best audio as base, we will postprocess to mp3 with bitrate
...
def run_download(job, url, fmt_key, filename):
    try:
        ...
        # If ffmpeg is present, set ffmpeg_location and set merge_output_format for mp4 jobs.
        if HAS_FFMPEG:
            opts["ffmpeg_location"] = ffmpeg_path()
            # For mp4_xxx use mp4 merge (remux). For audio jobs we will extract to mp3 via postprocessor.
            if fmt_key.startswith("mp4_"):
                opts["merge_output_format"] = "mp4"
            # For mp3 keys we add postprocessor to extract and set bitrate later
            if fmt_key.startswith("mp3_"):
                # preferredquality expects a number string like "192"
                bitrate_value = fmt_key.split("_", 1)[1]  # e.g., "128"
                # Use FFmpegExtractAudio postprocessor to get mp3 at desired kbps
                opts["postprocessors"] = [{
                    "key": "FFmpegExtractAudio",
                    "preferredcodec": "mp3",
                    "preferredquality": bitrate_value  # yt-dlp uses preferredquality as kbps value string
                }]
                # We also prefer mp3 extension in output template (but yt-dlp will handle)
                # No merge_output_format needed for audio-only
        else:
            # Without ffmpeg, audio bitrate change won't be available; warn user via job.error later if needed
            pass

...
<title>ðŸ’Ž Hyper Downloader</title>
<style>
...
<body>
  <div class="wrap">
    <header class="header">
      ...
      <nav>
        <a href="#features">ðŸ’Ž Premium</a>
        <a href="#faq">â“ FAQ</a>
    ...
    <section class="card">
      <h2>â¬‡ï¸ Download from YouTube</h2>
      <p class="lead">ðŸŽ¬ Paste your video link, choose format, and start downloading.</p>
      ...
      <form id="frm">
        ...
        <button id="goBtn" type="submit">âš¡ Start Download</button>
      </form>
...
fetch("/env").then(r=>r.json()).then(j=>{
  HAS_FFMPEG=!!j.ffmpeg;
  if(!HAS_FFMPEG){msg.textContent="âš ï¸ FFmpeg missing â€” MP3/merge disabled.";msg.style.color="#f59e0b";}
});
...
async function fetchInfo(url){
  try{
    ...
    pSub.textContent=[j.channel,j.duration_str].filter(Boolean).join(" â€¢ ");
...
async function poll(){
  ...
  try{
    ...
    if(p.status==="finished"){msg.textContent="âœ… Done â€” preparing file...";window.location="/fetch/"+job;job=null;return;}
    else if(p.status==="error"){msg.textContent="âŒ "+(p.error||"Download failed");job=null;return;}
    setTimeout(poll,700);

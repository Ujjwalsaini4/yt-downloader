...
# ---------- HTML (responsive + preview + ffmpeg-aware options) ----------
HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
...
fetch("/env").then(r=>r.json()).then(j=>{
  HAS_FFMPEG = !!j.ffmpeg;
  // Disable options that require ffmpeg if not available
  if(!HAS_FFMPEG){
    [...formatSel.options].forEach(o=>{
      if(o.dataset.needFfmpeg==="1"){ o.disabled = true; }
    });
    setMsg("FFmpeg not found: using single-file MP4 (lower quality). To enable 720p/1080p & MP3, install FFmpeg.", false);
  }
}).catch(()=>{});
...
frm.addEventListener("submit", async (e)=>{
  ...
  }catch(err){ setMsg(err.message||"Error starting download.", true); setBusy(false); }
});
...
async function poll(){
  ...
  try{
    ...
    if(r.status===404){ setMsg("Job expired or not found.", true); setBusy(false); job=null; return; }
    const p = await r.json();
    const pct = Math.max(0, Math.min(100, p.percent||0));
    bar.style.width = pct + "%"; document.querySelector(".progress").setAttribute("aria-valuenow", String(pct));
    if(p.status==="finished"){ setMsg("Done. Preparing your file…"); setBusy(false); window.location="/fetch/"+job; job=null; return; }
    if(p.status==="error"){ setMsg(p.error||"Download failed.", true); setBusy(false); job=null; return; }
    setTimeout(poll, 700);
  }catch(e){ setMsg("Network error while checking progress.", true); setBusy(false); job=null; }
}
</script>
</body>
</html>
...
def format_map_for_env():
    if HAS_FFMPEG:
        return {
            # Try bestvideo+bestaudio first; if not, fall back to single-file mp4
            "mp4_720"  : "bestvideo[ext=mp4][height<=720]+bestaudio[ext=m4a]/best[ext=mp4][height<=720]/best[ext=mp4]",
            "mp4_1080" : "bestvideo[ext=mp4][height<=1080]+bestaudio[ext=m4a]/best[ext=mp4][height<=1080]/best[ext=mp4]",
            "mp4_best" : "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]",
            "audio_mp3": "bestaudio/best"
        }
    else:
        # No merging possible: only single-file MP4 to avoid ffmpeg dependency
        return {
            "mp4_720"  : "best[ext=mp4][height<=720]/best[ext=mp4]",
            "mp4_1080" : "best[ext=mp4][height<=1080]/best[ext=mp4]",
            "mp4_best" : "best[ext=mp4]/best",
            # MP3 not supported without ffmpeg
            "audio_mp3": None
        }
...
def run_download(job: Job, url: str, fmt_key: str, name: str | None):
    try:
        ...
        if not fmt:
            job.status = "error"
            job.error = "This format requires FFmpeg. Install it or pick 'Best MP4'."
            return
        ...
        def hook(d):
            try:
                if d.get("status")=="downloading":
                    ...
                    job.percent = max(0, min(100, int(got*100/max(t,1))))
        ...
        # If no ffmpeg, DO NOT set postprocessors and use single-file formats only.
...
def start():
    ...
    fmt_choice = (d.get("format_choice") or "mp4_best").strip()
    filename = (d.get("filename") or "").strip() or None

    job = Job()
    threading.Thread(target=run_download, args=(job,url,fmt_choice,filename), daemon=True).start()
...
def info():
    ...
    try:
        yopts = {"skip_download": True, "quiet": True, "no_warnings": True, "noplaylist": True}
        with YoutubeDL(yopts) as y:
        ...
        def fmt_dur(s):
            ...
            return f"{h:01d}:{m:02d}:{sec:02d}" if h else f"{m:01d}:{sec:02d}"
        return jsonify({"title":title,"thumbnail":thumb,"channel":channel,"duration":dur,"duration_str":fmt_dur(dur)})
    except Exception as e:
        return jsonify({"error": str(e)[:180] or "Failed to fetch info"}), 400
...
def fetch(job_id):
    ...
    # Stream from disk (no RAM copy)
    resp = send_file(j.file, as_attachment=True, download_name=os.path.basename(j.file), conditional=True)
    # Cleanup after serving
    def _cleanup():
        try: shutil.rmtree(j.tmp, ignore_errors=True)
        finally: JOBS.pop(job_id, None)
